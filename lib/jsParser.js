...
// atok.debug(true).on('debug', console.log)
atok
	...
	.next('checkCmd').ignore()
		.match('eval(', ')', undefined, function (data) { handler(data, -1, 'eval') })
		.match('if(', ')', undefined, function (data) { handler(data, -1, 'ifStart') })
	.setDefaultHandler(handler) // TODO: bug workaround
		.addRule('else', 'ifElse')
		.addRule('endif', 'ifEnd')
